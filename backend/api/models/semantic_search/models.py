...
class SemanticSearchRequest(BaseModel):
    ...
    remote_types: Optional[List[str]] = Field(None, description="Filter by remote types")
    experience_levels: Optional[List[str]] = Field(None, description="Filter by experience levels")
...
class SemanticSearchResponse(BaseModel):
    ...
    results: List[SemanticSearchResult] = Field(..., description="List of search results")
    total: int = Field(..., description="Total number of results")
    limit: int = Field(..., description="Limit used for this search")
    processed_at: str = Field(..., description="Timestamp when the search was processed")
...
class HybridSearchRequest(BaseModel):
    ...
    location: Optional[str] = Field(None, description="Location filter")
    company: Optional[str] = Field(None, description="Company filter")
    keyword_weight: float = Field(0.5, description="Weight for keyword matching (0.0 to 1.0)", ge=0.0, le=1.0)
    semantic_weight: float = Field(0.5, description="Weight for semantic matching (0.0 to 1.0)", ge=0.0, le=1.0)
