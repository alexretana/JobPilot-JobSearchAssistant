...
class JobDeduplicationResponse(BaseModel):
    ...
    matching_fields: List[str] = Field(..., description="Which fields matched (title, company, etc.)")
    canonical_job_id: Optional[str] = Field(None, description="Canonical job ID if duplicates found")
    duplicate_job_id: Optional[str] = Field(None, description="Duplicate job ID if duplicates found")
    merge_strategy: str = Field("keep_canonical", description="Suggested merge strategy")
    processed_at: str = Field(..., description="Timestamp when the deduplication was processed")


class BatchDeduplicationRequest(BaseModel):
    """Request model for finding duplicates in a batch of jobs"""
    
    job_ids: List[str] = Field(..., description="List of job IDs to check for duplicates", min_items=2)
    confidence_threshold: float = Field(0.8, description="Minimum confidence score to consider duplicates", ge=0.0, le=1.0)
...
class BatchDeduplicationResponse(BaseModel):
    ...
    duplicates: List[BatchDeduplicationResult] = Field(..., description="List of duplicate pairs found")
    total_checked: int = Field(..., description="Total number of job pairs checked")
    total_duplicates_found: int = Field(..., description="Total number of duplicates found")
