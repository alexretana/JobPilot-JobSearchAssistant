...
class GeneralStatsResponse(BaseModel):
    ...
    active_users_last_24h: int = Field(..., description="Number of active users in the last 24 hours")
    new_jobs_last_24h: int = Field(..., description="Number of new jobs added in the last 24 hours")
    new_applications_last_24h: int = Field(..., description="Number of new applications in the last 24 hours")
...
class JobStatsResponse(BaseModel):
    ...
    top_companies: List[TopCompany] = Field(..., description="Top companies by job count")
    top_locations: List[TopLocation] = Field(..., description="Top locations by job count")
...
class UserStatsResponse(BaseModel):
    ...
    recent_registrations: int = Field(..., description="Number of new user registrations in the last 30 days")
    active_users_last_7_days: int = Field(..., description="Number of active users in the last 7 days")
    active_users_last_30_days: int = Field(..., description="Number of active users in the last 30 days")
    users_by_location: Dict[str, int] = Field(..., description="Count of users by location")
    average_profile_completion: float = Field(..., description="Average profile completion percentage")
    processed_at: datetime = Field(..., description="Timestamp when statistics were processed")
...
class ApplicationStatsResponse(BaseModel):
    ...
    total_applications: int = Field(..., description="Total number of job applications")
    applications_by_status: Dict[str, int] = Field(..., description="Count of applications by status")
    applications_by_job_type: Dict[str, int] = Field(..., description="Count of applications by job type")
    applications_by_experience_level: Dict[str, int] = Field(..., description="Count of applications by experience level")
    recent_applications: int = Field(..., description="Number of new applications in the last 30 days")
    applications_last_7_days: int = Field(..., description="Number of applications in the last 7 days")
    applications_last_30_days: int = Field(..., description="Number of applications in the last 30 days")
    average_time_to_apply: int = Field(..., description="Average time (in hours) from job posting to application")
    top_companies_applied: List[TopCompanyApplied] = Field(..., description="Top companies by application count")
    processed_at: datetime = Field(..., description="Timestamp when statistics were processed")
...
class ResumeStatsResponse(BaseModel):
    ...
    resumes_by_status: Dict[str, int] = Field(..., description="Count of resumes by status")
    recent_resumes: int = Field(..., description="Number of new resumes in the last 30 days")
    top_skills: List[TopSkillResume] = Field(..., description="Most commonly mentioned skills")
    average_resume_length: int = Field(..., description="Average resume length in words")
    processed_at: datetime = Field(..., description="Timestamp when statistics were processed")
...
class SkillBankStatsResponse(BaseModel):
    ...
    total_skill_banks: int = Field(..., description="Total number of skill banks")
    average_skills_per_bank: float = Field(..., description="Average number of skills per skill bank")
    top_skills: List[TopSkillBankSkill] = Field(..., description="Most commonly added skills")
    skill_categories: Dict[str, int] = Field(..., description="Count of skills by category")
    recent_skill_additions: int = Field(..., description="Number of new skill additions in the last 30 days")
    processed_at: datetime = Field(..., description="Timestamp when statistics were processed")
...
class JobSourceStatsResponse(BaseModel):
    ...
    sources_by_type: Dict[str, int] = Field(..., description="Count of sources by type (API, Scraping, Manual)")
    jobs_per_source: Dict[str, int] = Field(..., description="Average number of jobs per source")
    recently_scraped: int = Field(..., description="Number of sources scraped in the last 24 hours")
    processed_at: datetime = Field(..., description="Timestamp when statistics were processed")